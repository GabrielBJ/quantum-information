...
The input for the MVPO is data about the historical prices and returns of certain financial assets. An asset range $(1 \leq i \leq N)$ is considered, and a time range $(0 \leq t \leq T)$. 
...
\end{equation}
The goal of option pricing is to estimate the option payoff at the expiration date and then discount that value to determine the \textbf{fair value}, which indicates the amount of money an 
...
laverage some quantum mechanical principles to solve optimisation problems. The main goal of a VQA is to find an approximate solution for optimisation and combinatorial problems \cite{Cerezo2021}. Having stated the optimisation problem, 
we must map its cost function $C(\vec{\theta})$ to a Hamiltonian operator, whose ground state energy encodes the optimal solution.
...
\begin{figure}[h!]
\centering 
\includegraphics[width=0.45\textwidth]{VQE-Egger.pdf}
  \caption{\label{fig:vqe} Variational Quantum Eigensolver (VQE) algorithm.}
\end{figure}

One of this algorithms is the Variational Quantum Eigensolver 
